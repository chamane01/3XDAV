...
if uploaded_file:
    ...
    def clean_geojson(geojson_data):
        """Nettoie les propriétés d'un GeoJSON pour éviter les erreurs de sérialisation."""
        for feature in geojson_data['features']:
            if 'properties' in feature:
                feature['properties'] = {k: str(v) for k, v in feature['properties'].items()}
        return geojson_data

    # Conversion du GeoDataFrame en GeoJSON nettoyé
    geojson_data = json.loads(gdf.to_json())
    geojson_data = clean_geojson(geojson_data)
